...
const searchInternet = async (index: number) => {
  ...
  setResults(prev => prev.map((r, i) =>
    i === index ? { ...r, internetLoading: true } : r
  ))
    ...
    <div className="max-w-3xl mx-auto">
      ...
      {tokens === 0 && (
        <div className="bg-red-900/30 border border-red-500/30 rounded-xl p-4 mb-6">
          <p className="text-red-400 text-sm font-semibold">❌ No tokens available</p>
          <p className="text-red-300 text-xs mt-1">
            <a href="/dashboard/tokens" className="underline">Buy tokens</a> to run your TMA
          </p>
        </div>
      )}
      ...
      {phase === 'form' && (
        <div className="bg-gray-800 rounded-xl p-6 mb-6">
          <h3 className="font-bold text-white mb-4">Enter Your NOUN Credentials</h3>
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <p className="text-gray-400 text-sm">
              Balance: <span className="text-yellow-400 font-bold">🪙 {tokens}</span>
            </p>
          </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            <p className="text-green-400 font-semibold">
              ✅ {results.length} questions answered across {Object.keys(grouped).length} courses
            </p>
            <button
              onClick={() => { setPhase('form'); setResults([]) }}
              ...
              <div className="divide-y divide-gray-700">
                {questions.map((q, qi) => {
                  const globalIndex = results.findIndex(r =>
                    r.courseCode === q.courseCode && r.questionNumber === q.questionNumber
                  )
                  return (
      ...
      {phase === 'done' && results.length === 0 && (
        <div className="bg-gray-800 rounded-xl p-8 text-center">
          ...
          <p className="text-gray-400 text-sm">
            No active {tmaRound} found on your NOUN portal. Make sure your TMA is open.
          </p>
