...
function TokensContent() {
  ...
  return (
    <div className="max-w-xl mx-auto">
      <h2 className="text-2xl font-bold text-yellow-400 mb-6">🪙 Wallet & Tokens</h2>
      ...
      <div className="bg-gray-800 rounded-xl p-5 mb-4">
        ...
        <button onClick={handleTopUp} disabled={loading || topUpAmount < 100}
          className="w-full bg-yellow-500 text-gray-900 rounded-xl py-3 font-bold hover:bg-yellow-400 disabled:opacity-50">
          {loading ? 'Redirecting...' : `Pay ₦${topUpAmount.toLocaleString()} via Paystack`}
        </button>
      </div>
      ...
      <div className="bg-gray-800 rounded-xl p-5 mb-6">
        ...
        <div className="flex items-center gap-3 mb-3">
          <button onClick={() => setConvertAmount(Math.max(1, convertAmount - 1))}
            className="bg-gray-700 text-white w-10 h-10 rounded-xl font-bold text-lg hover:bg-gray-600">
            −
          </button>
          <div className="flex-1 text-center">
            <p className="text-3xl font-bold text-yellow-400">{convertAmount}</p>
            <p className="text-gray-400 text-xs">token{convertAmount > 1 ? 's' : ''}</p>
          </div>
          <button onClick={() => setConvertAmount(convertAmount + 1)}
            className="bg-gray-700 text-white w-10 h-10 rounded-xl font-bold text-lg hover:bg-gray-600">
            +
          </button>
        </div>
        ...
        <button onClick={handleConvert}
          ...
          className="w-full bg-yellow-500 text-gray-900 rounded-xl py-3 font-bold hover:bg-yellow-400 disabled:opacity-50">
          {converting ? 'Converting...' : `Convert to ${convertAmount} Token${convertAmount > 1 ? 's' : ''}`}
        </button>
      </div>
      ...
      <div className="bg-gray-800 rounded-xl p-5">
        ...
        {transactions.length === 0 ? (
          ...
          <div className="space-y-3">
            {transactions.map(t => (
              <div key={t.id} className="flex items-center justify-between p-3 bg-gray-700/50 rounded-lg">
                <div>
                  <p className="text-sm font-medium text-white">{t.description}</p>
                  <p className="text-gray-400 text-xs">{new Date(t.created_at).toLocaleDateString()}</p>
                </div>
                <span className={`font-bold text-sm ${t.type === 'credit' ? 'text-green-400' : 'text-red-400'}`}>
